...
export function generatePeriods(dateFrom, dateTo, granularity) {
  ...
  if (granularity === 'daywise') {
    // 7 buckets: Mon through Sun — each collects all matching days from the range
    const dayNames = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    const dayDates = { 1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 0:[] }
    let cur = new Date(start)
    while (cur <= end) {
      const dow = cur.getDay()
      dayDates[dow].push(cur.toISOString().split('T')[0])
    ...
    order.forEach(dow => {
      ...
      if (dates.length) {
        periods.push({
          key: dayNames[dow === 0 ? 6 : dow - 1],
          label: `${dayNames[dow === 0 ? 6 : dow - 1]}\n(${dates.length} days)`,
          dates: dates, // special: array of individual dates
        })
      }
    })
    return periods
...
export async function fetchPeriodData(brand, dateFrom, dateTo, granularity) {
  ...
  // For daywise, filter by specific dates instead of from/to range
  function filterDates(rows, dates) {
    return rows.filter(r => dates.includes(r.date))
  }

  return periods.map(p => {
    let zs, ss, zr, sr
    if (p.dates) {
      // daywise — filter by array of specific dates
      zs = filterDates(zomatoSummary, p.dates)
      ss = filterDates(swiggySummary, p.dates)
      zr = filterDates(zomatoReport, p.dates)
      sr = filterDates(swiggyReport, p.dates)
    } else {
